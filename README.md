...
* 클래스 정의할 떄는 `models.Model` 을 상속받아 만든다.
...
* 특정 데이터 조회
  ...
  Article.objects.filter(title_contains='제목') # 제목이 들어간 제목
  Article.objects.filter(title_startswith='제목') # 제목으로 시작하는 제목
  Article.objects.filter(title_endswith='제목') # 제목으로 끝나는 제목
  ```
